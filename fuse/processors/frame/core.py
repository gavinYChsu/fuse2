...
from fuse.typing import Process_Frames
...
def load_frame_processor_module(frame_processor : str) -> Any:
	try:
		frame_processor_module = importlib.import_module('fuse.processors.frame.modules.' + frame_processor)
...
def multi_process_frames(source_path : str, temp_frame_paths : List[str], process_frames : Process_Frames) -> None:
	...
	with tqdm(total = len(temp_frame_paths), desc = wording.get('processing'), unit = 'frame', dynamic_ncols = True, bar_format = progress_bar_format) as progress:
		with ThreadPoolExecutor(max_workers = fuse.globals.execution_thread_count) as executor:
			futures = []
			queue_temp_frame_paths : Queue[str] = create_queue(temp_frame_paths)
			queue_per_future = max(len(temp_frame_paths) // fuse.globals.execution_thread_count * fuse.globals.execution_queue_count, 1)
...
def update_progress(progress : Any = None) -> None:
	...
	progress.set_postfix(
	{
		...
		'execution_providers': fuse.globals.execution_providers,
		'execution_thread_count': fuse.globals.execution_thread_count,
		'execution_queue_count': fuse.globals.execution_queue_count
	})
	progress.refresh()
	progress.update(1)
