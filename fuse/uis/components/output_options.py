...
from fuse.typing import OutputVideoEncoder
...
def render() -> None:
	...
	OUTPUT_PATH_TEXTBOX = gradio.Textbox(
		...
		value = fuse.globals.output_path or tempfile.gettempdir(),
		max_lines = 1
	)
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('output_image_quality_slider_label'),
		value = fuse.globals.output_image_quality,
		step = 1,
		minimum = 0,
		maximum = 100,
		visible = is_image(fuse.globals.target_path)
	)
	OUTPUT_VIDEO_ENCODER_DROPDOWN = gradio.Dropdown(
		label = wording.get('output_video_encoder_dropdown_label'),
		choices = fuse.choices.output_video_encoders,
		value = fuse.globals.output_video_encoder,
		visible = is_video(fuse.globals.target_path)
	)
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('output_video_quality_slider_label'),
		value = fuse.globals.output_video_quality,
		step = 1,
		minimum = 0,
		maximum = 100,
		visible = is_video(fuse.globals.target_path)
...
def update_output_path(output_path : str) -> None:
	fuse.globals.output_path = output_path


def update_output_image_quality(output_image_quality : int) -> None:
	fuse.globals.output_image_quality = output_image_quality


def update_output_video_encoder(output_video_encoder: OutputVideoEncoder) -> None:
	fuse.globals.output_video_encoder = output_video_encoder


def update_output_video_quality(output_video_quality : int) -> None:
	fuse.globals.output_video_quality = output_video_quality
