...
def render() -> None:
	...
	is_target_image = is_image(fuse.globals.target_path)
	is_target_video = is_video(fuse.globals.target_path)
	TARGET_FILE = gradio.File(
		...
		value = fuse.globals.target_path if is_target_image or is_target_video else None
	)
...
def update(file : IO[Any]) -> Tuple[gradio.Image, gradio.Video]:
	...
	if file and is_image(file.name):
		fuse.globals.target_path = file.name
