...
from fuse.vision import get_video_frame, normalize_frame_color, read_static_image
from fuse.face_analyser import get_many_faces
from fuse.face_reference import clear_face_reference
from fuse.typing import Frame, FaceRecognition
from fuse.utilities import is_image, is_video
...
def render() -> None:
	...
	reference_face_gallery_args: Dict[str, Any] =\
	{
		...
		'columns': 10,
		'allow_preview': False,
		'visible': 'reference' in fuse.globals.face_recognition
	}
	if is_image(fuse.globals.target_path):
		reference_frame = read_static_image(fuse.globals.target_path)
	...
	FACE_RECOGNITION_DROPDOWN = gradio.Dropdown(
		...
		choices = fuse.choices.face_recognitions,
		value = fuse.globals.face_recognition
	)
	REFERENCE_FACE_POSITION_GALLERY = gradio.Gallery(**reference_face_gallery_args)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('reference_face_distance_slider_label'),
		value = fuse.globals.reference_face_distance,
		step = 0.05,
		minimum = 0,
		maximum = 3,
		visible = 'reference' in fuse.globals.face_recognition
...
def update_face_recognition(face_recognition : FaceRecognition) -> Tuple[gradio.Gallery, gradio.Slider]:
	if face_recognition == 'reference':
		fuse.globals.face_recognition = face_recognition
...
def update_face_reference_position(reference_face_position : int = 0) -> gradio.Gallery:
	gallery_frames = []
	fuse.globals.reference_face_position = reference_face_position
	if is_image(fuse.globals.target_path):
		reference_frame = read_static_image(fuse.globals.target_path)
...
def update_reference_face_distance(reference_face_distance : float) -> None:
	fuse.globals.reference_face_distance = reference_face_distance
