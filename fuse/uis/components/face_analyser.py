...
def render() -> None:
	...
	FACE_ANALYSER_DIRECTION_DROPDOWN = gradio.Dropdown(
		...
		choices = fuse.choices.face_analyser_directions,
		value = fuse.globals.face_analyser_direction
	)
	FACE_ANALYSER_AGE_DROPDOWN = gradio.Dropdown(
		label = wording.get('face_analyser_age_dropdown_label'),
		choices = ['none'] + fuse.choices.face_analyser_ages,
		value = fuse.globals.face_analyser_age or 'none'
	)
	FACE_ANALYSER_GENDER_DROPDOWN = gradio.Dropdown(
		label = wording.get('face_analyser_gender_dropdown_label'),
		choices = ['none'] + fuse.choices.face_analyser_genders,
		value = fuse.globals.face_analyser_gender or 'none'
	)
...
def update_dropdown(name : str, value : str) -> None:
	if value == 'none':
		setattr(fuse.globals, name, None)
	else:
		setattr(fuse.globals, name, value)
