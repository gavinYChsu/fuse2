...
from fuse.processors.frame.core import load_frame_processor_module, clear_frame_processors_modules
from fuse.utilities import list_module_names
...
def render() -> None:
	...
	FRAME_PROCESSORS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = sort_frame_processors(fuse.globals.frame_processors),
		value = fuse.globals.frame_processors
	)
...
def update_frame_processors(frame_processors : List[str]) -> gradio.CheckboxGroup:
	fuse.globals.frame_processors = frame_processors
