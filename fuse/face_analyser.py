...
import fuse.globals
from fuse.face_cache import get_faces_cache, set_faces_cache
from fuse.typing import Frame, Face, FaceAnalyserDirection, FaceAnalyserAge, FaceAnalyserGender
...
def get_face_analyser() -> Any:
	...
	with THREAD_LOCK:
		if FACE_ANALYSER is None:
			FACE_ANALYSER = insightface.app.FaceAnalysis(name = 'buffalo_l', providers = fuse.globals.execution_providers)
...
def get_many_faces(frame : Frame) -> List[Face]:
	try:
		...
		if fuse.globals.face_analyser_direction:
			faces = sort_by_direction(faces, fuse.globals.face_analyser_direction)
		if fuse.globals.face_analyser_age:
			faces = filter_by_age(faces, fuse.globals.face_analyser_age)
		if fuse.globals.face_analyser_gender:
			faces = filter_by_gender(faces, fuse.globals.face_analyser_gender)
		return faces
