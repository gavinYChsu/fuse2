...
from fuse.vision import detect_fps

#TEMP_DIRECTORY_PATH = os.path.join(tempfile.gettempdir(), 'fuse')
#TEMP_DIRECTORY_PATH = '/content/drive/MyDrive/FSWAP'
#TEMP_DIRECTORY_PATH = fuse.globals.temp_path
...
def extract_frames(target_path : str, fps : float) -> bool:
	temp_frame_compression = round(31 - (fuse.globals.temp_frame_quality * 0.31))
	trim_frame_start = fuse.globals.trim_frame_start
	trim_frame_end = fuse.globals.trim_frame_end
...
def compress_image(output_path : str) -> bool:
	output_image_compression = round(31 - (fuse.globals.output_image_quality * 0.31))
...
def merge_video(target_path : str, fps : float) -> bool:
	...
	temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d')
	commands = [ '-hwaccel', 'auto', '-r', str(fps), '-i', temp_frames_pattern, '-c:v', fuse.globals.output_video_encoder ]
	if fuse.globals.output_video_encoder in [ 'libx264', 'libx265' ]:
		output_video_compression = round(51 - (fuse.globals.output_video_quality * 0.51))
		commands.extend([ '-crf', str(output_video_compression) ])
	if fuse.globals.output_video_encoder in [ 'libvpx-vp9' ]:
		output_video_compression = round(63 - (fuse.globals.output_video_quality * 0.63))
		commands.extend([ '-crf', str(output_video_compression) ])
	if fuse.globals.output_video_encoder in [ 'h264_nvenc', 'hevc_nvenc' ]:
		output_video_compression = round(51 - (fuse.globals.output_video_quality * 0.51))
...
def get_temp_frames_pattern(target_path : str, temp_frame_prefix : str) -> str:
	...
	return os.path.join(temp_directory_path, temp_frame_prefix + '.' + fuse.globals.temp_frame_format)


def get_temp_directory_path(target_path : str) -> str:
	target_name, _ = os.path.splitext(os.path.basename(target_path))
	return os.path.join(fuse.globals.temp_path, target_name)
...
def clear_temp(target_path : str) -> None:
	...
	if not fuse.globals.keep_temp and is_directory(temp_directory_path):
...
def get_processed_frame_count(target_path) -> int:
	...
	target_name, _ = os.path.splitext(os.path.basename(target_path))
	fp_count = os.path.join(fuse.globals.temp_path, f"{target_name}_count.txt")
