...
import fuse.globals
from fuse.utilities import conditional_download, extract_frames, create_temp, get_temp_directory_path, clear_temp, normalize_output_path, is_file, is_directory, is_image, is_video, get_download_size, is_download_done, encode_execution_providers, decode_execution_providers


@pytest.fixture(scope = 'module', autouse = True)
def before_all() -> None:
	fuse.globals.temp_frame_quality = 100
	fuse.globals.trim_frame_start = None
	fuse.globals.trim_frame_end = None
	fuse.globals.temp_frame_format = 'png'
	conditional_download('.assets/examples',
	[
		'https://github.com/fuse/fuse-assets/releases/download/examples/source.jpg',
		'https://github.com/fuse/fuse-assets/releases/download/examples/target-240p.mp4'
	])
...
def before_each() -> None:
	fuse.globals.trim_frame_start = None
	fuse.globals.trim_frame_end = None
	fuse.globals.temp_frame_quality = 90
	fuse.globals.temp_frame_format = 'jpg'
...
def test_extract_frames_with_trim_start() -> None:
	fuse.globals.trim_frame_start = 224
	data_provider =\
	[
...
def test_extract_frames_with_trim_start_and_trim_end() -> None:
	fuse.globals.trim_frame_start = 124
	fuse.globals.trim_frame_end = 224
	data_provider =\
	[
...
def test_extract_frames_with_trim_end() -> None:
	fuse.globals.trim_frame_end = 100
	data_provider =\
	[
		('.assets/examples/target-240p-25fps.mp4', 120),
		('.assets/examples/target-240p-30fps.mp4', 100),
...
def test_get_download_size() -> None:
	assert get_download_size('https://github.com/fuse/fuse-assets/releases/download/examples/target-240p.mp4') == 191675
	assert get_download_size('https://github.com/fuse/fuse-assets/releases/download/examples/target-360p.mp4') == 370732
	assert get_download_size('invalid') == 0


def test_is_download_done() -> None:
	assert is_download_done('https://github.com/fuse/fuse-assets/releases/download/examples/target-240p.mp4', '.assets/examples/target-240p.mp4') is True
	assert is_download_done('https://github.com/fuse/fuse-assets/releases/download/examples/target-240p.mp4','invalid') is False
